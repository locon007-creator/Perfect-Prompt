export function findCriticalGaps(intent,taskType='general'){
  ...
  if(taskType==='decompiler' && /\badapt\b/i.test(intent.idea) && !/\b(?:from|original).*\b(?:to|for)\b/i.test(intent.idea)) gaps.push({dimension:'context',question:'What tool is the original prompt from, and what tool should it be adapted for?'});
